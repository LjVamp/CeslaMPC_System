...
const playOrderSound = async () => {
  if (Platform.OS === 'web' && typeof window !== 'undefined' && window.AudioContext) {
    try {
      ...
      playTone(1046, 0.00, 1.2, 0.9);
      playTone(1318, 0.00, 1.0, 0.5);
...
const WebScrollView = ({ children, style, contentContainerStyle, ...rest }) => {
  ...
  return (
    <View style={[{ flex:1, minHeight:0, position:'relative', overflow:'hidden' }, style]}>
      <div className="cesla-grocery-scroll" style={{
...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const emptyItem = () => ({
  id: Date.now().toString(), name:'', cat:'Produce',
...
const ItemEditModal = ({ visible, item, categories, onSave, onClose }) => {
  ...
  const pickImage = async () => {
    ...
    const res = await ImagePicker.launchImageLibraryAsync({
      ...
      allowsEditing: true, aspect: [1, 1], quality: 0.5, base64: false,
    });
    if (!res.canceled) {
            ...
            <View style={{ flexDirection:'row', gap:12, alignItems:'flex-start' }}>
              ...
              <View style={{flex:1, gap:8}}>
                <View style={ms.fieldRow}>
                  ...
                  <TextInput style={ms.input} value={form.name} onChangeText={handleNameChange} placeholder="e.g. White Rice 1kg"/>
                </View>
                <View style={ms.fieldRow}>
...
// ─── MANAGE ITEMS SCREEN ──────────────────────────────────────────────────────
const ManageItemsScreen = ({ items, categories, filtered, search, activeCategory, onSearch, onCategoryChange, onAddItem, onEditItem, onDeleteItem, isWide: mmIsWide }) => {
...
const InventoryScreen = ({ items, maxQtyMap, onAddItem, onEditItem }) => {
  ...
  const formatDateLabel = (d) => {
    ...
    if (d.toDateString() === now.toDateString()) return "Today's Stocks, " + d.toLocaleDateString('en-PH', opts);
    if (d.toDateString() === new Date(now - 86400000).toDateString()) return "Yesterday's Stocks, " + d.toLocaleDateString('en-PH', opts);
  ...
  return (
    <View style={{ flex:1, minHeight:0, overflow:'hidden', position:'relative' }}>
      <View style={{ flexDirection:'row', alignItems:'center', gap:8, marginHorizontal:14, marginTop:8, marginBottom:6 }}>
        <TouchableOpacity style={inv.titleRow} onPress={() => setShowDatePicker(p => !p)} activeOpacity={0.80}>
          <Text style={inv.titleText}>{formatDateLabel(selectedDate)}</Text>
          <Text style={inv.titleCaret}>{showDatePicker ? '▲' : '▼'}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={inv.addItemBtn} onPress={() => onAddItem && onAddItem()} activeOpacity={0.80}>
      ...
      {showDatePicker && (
        <View style={inv.calCard}>
          <View style={inv.calNav}>
            <TouchableOpacity style={inv.calNavBtn} onPress={() => { if(calMonth===0){setCalMonth(11);setCalYear(y=>y-1);}else setCalMonth(m=>m-1); }}><Text style={inv.calNavTxt}>‹</Text></TouchableOpacity>
            <Text style={inv.calMonthLbl}>{MONTHS[calMonth]} {calYear}</Text>
            <TouchableOpacity style={inv.calNavBtn} onPress={() => { if(calMonth===11){setCalMonth(0);setCalYear(y=>y+1);}else setCalMonth(m=>m+1); }}><Text style={inv.calNavTxt}>›</Text></TouchableOpacity>
          </View>
          <View style={inv.calDaysRow}>{DAYS.map(d=><Text key={d} style={inv.calDayHdr}>{d}</Text>)}</View>
          <View style={inv.calGrid}>
            {calDays.map((day,idx)=>{
              ...
              return(
                <TouchableOpacity key={idx} style={[inv.calCell,isSelected&&inv.calCellSel,isToday&&!isSelected&&inv.calCellToday]}
                  onPress={()=>{setSelectedDate(new Date(calYear,calMonth,day));setShowDatePicker(false);}}>
                  <Text style={[inv.calCellTxt,isSelected&&inv.calCellTxtSel]}>{day}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
      ...
      <View style={inv.tableWrap}>
        ...
        <WebScrollView style={{flex:1}} contentContainerStyle={{gap:0}}>
          {items.map((item,idx)=>{
            ...
            return(
              <TouchableOpacity key={item.id} style={[inv.trow,idx%2===0&&inv.trowAlt]} onPress={()=>onEditItem&&onEditItem(item)} activeOpacity={0.75}>
                <View style={[inv.td,inv.colName]}><Text style={inv.tdName} numberOfLines={1}>{item.emoji}  {item.name}</Text></View>
                <View style={[inv.td,inv.colCat]}><Text style={inv.tdMuted} numberOfLines={1}>{item.cat}</Text></View>
                <View style={[inv.td,inv.colQty]}><Text style={[inv.tdNum,item.stock===0&&{color:'#e74c3c',fontFamily:'GoogleSans_700Bold'},item.stock<=5&&item.stock>0&&{color:'#b85c00',fontFamily:'GoogleSans_700Bold'}]}>{item.stock}</Text></View>
                <View style={[inv.td,inv.colMaxQty]}><Text style={[inv.tdNum,{textAlign:'center'}]}>{max}</Text></View>
                <View style={[inv.td,inv.colPrice]}><Text style={inv.tdNum}>₱{item.price.toLocaleString()}</Text></View>
                <View style={[inv.td,inv.colValue]}><Text style={[inv.tdNum,{color:'#1a3a6b',fontFamily:'GoogleSans_700Bold'}]}>₱{(item.price*item.stock).toLocaleString()}</Text></View>
                <View style={[inv.td,inv.colRestock]}>
                  {restock>0?(<View style={inv.restockBadge}><Text style={inv.restockNeed}>Need {restock}</Text><Text style={inv.restockSub}>({item.stock}/{max})</Text></View>):(<Text style={inv.restockOk}>✓ OK</Text>)}
                </View>
              </TouchableOpacity>
            );
          })}
        ...
        <View style={inv.tfooter}>
          <View style={[inv.td,inv.colName]}><Text style={inv.tfootLbl}>TOTALS</Text></View>
          <View style={[inv.td,inv.colCat]}/>
          <View style={[inv.td,inv.colQty]}><Text style={[inv.tfootVal,{textAlign:'center'}]}>{overallQty}</Text></View>
          <View style={[inv.td,inv.colMaxQty]}/>
          <View style={[inv.td,inv.colPrice]}/>
          <View style={[inv.td,inv.colValue]}><Text style={[inv.tfootVal,{color:'#8a6500',textAlign:'center'}]}>₱{grandTotal.toLocaleString()}</Text></View>
          <View style={[inv.td,inv.colRestock]}/>
        </View>
      </View>
    </View>
...
const OrderHistoryScreen = ({ orders }) => {
  ...
  const parseOrderDate = (o) => {
    ...
    if (o.time) { const d=new Date(o.time); return isNaN(d)?null:d; }
    return null;
  };
  ...
  const grouped=React.useMemo(()=>{
    const map={};
    [...orders].forEach(o=>{const d=parseOrderDate(o);const k=dateKey(d);if(!map[k])map[k]={key:k,date:d,orders:[]};map[k].orders.push(o);});
    Object.values(map).forEach(g=>g.orders.sort((a,b)=>(parseOrderDate(b)?.getTime()||0)-(parseOrderDate(a)?.getTime()||0)));
        ...
        <View style={hst.calCard}>
          ...
          <View style={inv.calGrid}>
            {calDays.map((day,idx)=>{
              ...
              return(<TouchableOpacity key={idx} style={[inv.calCell,isSel&&inv.calCellSel,isToday&&!isSel&&inv.calCellToday,!hasOrders&&{opacity:0.30}]} onPress={()=>{if(hasOrders){setSelectedDate(dk);setShowCalendar(false);}}} activeOpacity={hasOrders?0.75:1}>
                <Text style={[inv.calCellTxt,isSel&&inv.calCellTxtSel]}>{day}</Text>
      ...
      {displayOrders.length===0
        ...
        :<WebScrollView contentContainerStyle={{gap:4,paddingBottom:20}}>
          {displayOrders.map((order,idx)=>{
            ...
            const timeOnly=d?d.toLocaleTimeString('en-PH',{hour:'2-digit',minute:'2-digit'}):(order.time||'');
            ...
            return(<View key={order.id||order.docId} style={hst.txRow}>
              ...
              <View style={hst.txContent}>
                <View style={{flexDirection:'row',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                  ...
                  {order.memberName&&<Text style={{fontFamily:'GoogleSans_400Regular',fontSize:9,color:'rgba(1,31,75,0.55)'}}>👤 {order.memberName}</Text>}
                </View>
...
const CreditsScreen = () => {
  ...
  const markSettled = async (docId) => {
    setSettlingId(docId);
    try {
      await updateDoc(doc(db, 'grocery_orders', docId), { settled:true, settledAt:serverTimestamp() });
    } catch (e) {
  ...
  return (
    <View style={sub.root}>
      <View style={{flexDirection:'row',alignItems:'center',backgroundColor:'rgba(255,255,255,0.65)',borderRadius:10,paddingHorizontal:12,paddingVertical:9,margin:16,marginBottom:10,borderWidth:1.5,borderColor:'rgba(255,255,255,0.90)',gap:8}}>
        ...
        <TextInput style={{flex:1,fontFamily:'GoogleSans_400Regular',fontSize:13,color:'#0f1e35'}} value={search} onChangeText={setSearch} placeholder="Search member name or ID..." placeholderTextColor="rgba(1,31,75,0.35)" autoCorrect={false}/>
        {search.length>0&&<TouchableOpacity onPress={()=>setSearch('')}><Text style={{color:'rgba(1,31,75,0.45)',fontSize:14}}>✕</Text></TouchableOpacity>}
      </View>
      ...
      <ScrollView style={{flex:1}} contentContainerStyle={{paddingHorizontal:16,paddingBottom:32}} showsVerticalScrollIndicator={false}>
        {filtered.length===0?(
          <View style={{alignItems:'center',paddingVertical:60}}>
            ...
            <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:15,color:'rgba(1,31,75,0.45)',textAlign:'center'}}>{search?'No members match your search.':'No credit orders yet.'}</Text>
          </View>
        ):filtered.map(group=>{
          ...
          return(
            <TouchableOpacity key={group.memberId||group.memberName}
              onPress={()=>{setSelectedMember(group.memberId||group.memberName);setActiveModalTab('unpaid');}}
              activeOpacity={0.80}
              style={{backgroundColor:'rgba(255,255,255,0.55)',borderRadius:14,marginBottom:8,borderWidth:1.5,borderColor:isSettled?'rgba(39,174,96,0.35)':'rgba(201,168,76,0.40)',overflow:'hidden',flexDirection:'row',alignItems:'center',gap:12,padding:14,paddingLeft:18}}>
              ...
              <View style={{width:44,height:44,borderRadius:22,backgroundColor:isSettled?'rgba(39,174,96,0.15)':'rgba(201,168,76,0.20)',borderWidth:2,borderColor:isSettled?'#27ae60':'#c9a84c',justifyContent:'center',alignItems:'center',flexShrink:0}}>
                <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:15,color:isSettled?'#27ae60':'#c9a84c'}}>{(group.memberName||'?').split(' ').map(w=>w[0]).slice(0,2).join('')}</Text>
              </View>
              <View style={{flex:1,minWidth:0}}>
                ...
                <View style={{flexDirection:'row',gap:10,marginTop:4}}>
                  ...
                  <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:10,color:'#27ae60'}}>✅ {group.orders.filter(o=>o.settled===true).length} paid</Text>
                </View>
              </View>
              <View style={{alignItems:'flex-end',gap:4}}>
                <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:18,color:isSettled?'#27ae60':'#c9a84c'}}>₱ {totalOwed.toFixed(2)}</Text>
                {isSettled&&<View style={{backgroundColor:'rgba(39,174,96,0.15)',borderRadius:10,paddingHorizontal:8,paddingVertical:2,borderWidth:1,borderColor:'rgba(39,174,96,0.40)'}}><Text style={{fontFamily:'GoogleSans_700Bold',fontSize:9,color:'#27ae60'}}>✓ SETTLED</Text></View>}
              </View>
            </TouchableOpacity>
          );
        })}
      ...
      <Modal transparent visible={!!selectedMember} animationType="fade" onRequestClose={()=>setSelectedMember(null)}>
        <View style={{flex:1,backgroundColor:'rgba(1,15,40,0.55)',justifyContent:'center',alignItems:'center',padding:16}}>
          ...
          <View style={{width:'100%',maxWidth:700,maxHeight:'88%',backgroundColor:'#f0f5f9',borderRadius:20,shadowColor:'#000',shadowOpacity:0.30,shadowRadius:24,elevation:20,overflow:'hidden'}}>
            <LinearGradient colors={['#1a2d4e','#243554']} style={{padding:16,flexDirection:'row',alignItems:'center',gap:12}}>
              <View style={{width:42,height:42,borderRadius:21,backgroundColor:'rgba(201,168,76,0.25)',borderWidth:2,borderColor:'#c9a84c',justifyContent:'center',alignItems:'center'}}>
                <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:15,color:'#c9a84c'}}>{(modalGroup?.memberName||'?').split(' ').map(w=>w[0]).slice(0,2).join('')}</Text>
              </View>
              <View style={{flex:1}}>
              ...
              <TouchableOpacity onPress={()=>setSelectedMember(null)} style={{width:32,height:32,borderRadius:16,backgroundColor:'rgba(255,255,255,0.15)',justifyContent:'center',alignItems:'center'}}>
                <Text style={{color:'#fff',fontSize:14}}>✕</Text>
              </TouchableOpacity>
            </LinearGradient>

            <View style={{flexDirection:'row',gap:10,padding:14,paddingBottom:0}}>
              {[['unpaid','⏳ Unpaid',totalUnpaid,unpaidOrders.length,'#1a2d4e','#c9a84c','rgba(201,168,76,0.15)'],
                ['paid','✅ Paid',totalPaid,paidOrders.length,'#1a4a2e','#4cde8a','rgba(39,174,96,0.10)']].map(([key,label,amt,cnt,activeBg,activeColor,inactiveBg])=>(
                <TouchableOpacity key={key} onPress={()=>setActiveModalTab(key)} activeOpacity={0.85}
                  style={{flex:1,borderRadius:12,padding:12,backgroundColor:activeModalTab===key?activeBg:'rgba(255,255,255,0.55)',borderWidth:1.5,borderColor:activeModalTab===key?(key==='unpaid'?'#c9a84c':'#27ae60'):'rgba(255,255,255,0.70)'}}>
                  <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:10,color:activeModalTab===key?'rgba(255,255,255,0.55)':'rgba(1,31,75,0.45)',letterSpacing:1.2,textTransform:'uppercase',marginBottom:4}}>{label}</Text>
                  <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:22,color:activeModalTab===key?activeColor:(key==='unpaid'?'#e67e22':'#27ae60')}}>₱ {amt.toFixed(2)}</Text>
                  <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:activeModalTab===key?'rgba(255,255,255,0.45)':'rgba(1,31,75,0.45)',marginTop:3}}>{cnt} order{cnt!==1?'s':''}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <ScrollView style={{flex:1,margin:14,marginTop:12}} showsVerticalScrollIndicator={false} nestedScrollEnabled>
              <View style={{flexDirection:'row',backgroundColor:'#1a2d4e',borderRadius:8,paddingVertical:9,paddingHorizontal:8,marginBottom:4}}>
                {[{l:'ORDER NO.',f:0.8},{l:'DATE/TIME',f:1.3},{l:'ITEMS',f:2.0},{l:'QTY',f:0.5},{l:'PRICE',f:0.7},{l:'AMOUNT',f:0.8},{l:'',f:0.8}].map(col=>(
                  <Text key={col.l} style={{fontFamily:'GoogleSans_700Bold',fontSize:9,color:'#c9a84c',letterSpacing:1.2,textTransform:'uppercase',flex:col.f,textAlign:col.l==='AMOUNT'||col.l==='QTY'||col.l==='PRICE'?'right':'left'}}>{col.l}</Text>
                ))}
              </View>
              ...
              {(activeModalTab==='unpaid'?unpaidOrders:paidOrders).length===0?(
                <View style={{alignItems:'center',paddingVertical:40}}>
                  ...
                  <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:13,color:'rgba(1,31,75,0.45)',textAlign:'center'}}>{activeModalTab==='unpaid'?'No unpaid orders!':'No paid orders yet.'}</Text>
                </View>
                ...
                return [
                  ...orderItems.map((it,j)=>{
                    const item=it.item||it; const qty=it.qty||it.quantity||1; const lineAmt=Number(item.price||0)*qty; const isFirst=j===0;
                    return(
                      <View key={`${order.docId}-${j}`} style={{flexDirection:'row',alignItems:'center',paddingVertical:8,paddingHorizontal:8,backgroundColor:rowBg,borderTopLeftRadius:isFirst?8:0,borderTopRightRadius:isFirst?8:0,borderBottomWidth:1,borderColor:'rgba(1,31,75,0.06)'}}>
                        <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:11,color:isFirst?'#0f1e35':'transparent',flex:0.8}}>{isFirst?`#${order.orderNo||'—'}`:''}</Text>
                        <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:9.5,color:isFirst?'rgba(1,31,75,0.55)':'transparent',flex:1.3,lineHeight:14}}>{isFirst?fmtDateTime(order.createdAt):''}</Text>
                        <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:'#0f1e35',flex:2.0}} numberOfLines={1}>{item.emoji||'🛒'} {item.name||'—'}</Text>
                        <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:11,color:'#0f1e35',flex:0.5,textAlign:'right'}}>{qty}</Text>
                        <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:'rgba(1,31,75,0.55)',flex:0.7,textAlign:'right'}}>₱{Number(item.price||0).toFixed(2)}</Text>
                        <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:'rgba(1,31,75,0.65)',flex:0.8,textAlign:'right'}}>₱{lineAmt.toFixed(2)}</Text>
                        <View style={{flex:0.8}}/>
                      </View>
                    );
                  }),
                  <View key={`${order.docId}-subtotal`} style={{flexDirection:'row',alignItems:'center',paddingVertical:8,paddingHorizontal:8,backgroundColor:rowBg,borderBottomLeftRadius:8,borderBottomRightRadius:8,marginBottom:6,borderTopWidth:1,borderColor:'rgba(1,31,75,0.12)'}}>
                    <Text style={{flex:0.8}}/><Text style={{flex:1.3}}/>
                    ...
                    <View style={{flex:0.8,alignItems:'flex-end'}}>
                      {!isPaid?(
                        ...
                        <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:9,color:'#27ae60',textAlign:'right'}}>✓ {fmtDateTime(order.settledAt)}</Text>
                      ):null}
                    </View>
                  </View>,
                ];
...
const SalesReportScreen = ({ orders, items }) => {
  ...
  const yearOrders  = orders.filter(o=>{const d=getDate(o);return d&&d.getFullYear()===year;});
  const monthOrders = yearOrders.filter(o=>{const d=getDate(o);return d&&d.getMonth()===activeMonth;});

  const dailyData = React.useMemo(()=>{
    const map={};
    monthOrders.forEach(o=>{const d=getDate(o);if(!d)return;const k=`${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}-${d.getFullYear()}`;if(!map[k])map[k]={key:k,total:0,count:0};map[k].total+=Number(o.total||0);map[k].count+=1;});
    return Object.values(map).sort((a,b)=>a.key.localeCompare(b.key));
  },[monthOrders]);

  const monthlyData = React.useMemo(()=>{
    return MONTHS.map((m,i)=>{const mo=yearOrders.filter(o=>{const d=getDate(o);return d&&d.getMonth()===i;});return{month:m,monthFull:MONTHS_FULL[i],total:mo.reduce((s,o)=>s+Number(o.total||0),0),count:mo.length};});
  },[yearOrders]);

  const yearlyData = React.useMemo(()=>{
    const map={};
    orders.forEach(o=>{const d=getDate(o);if(!d)return;const y=d.getFullYear();if(!map[y])map[y]={year:y,total:0,count:0};map[y].total+=Number(o.total||0);map[y].count+=1;});
  ...
  const Controls = ({ view, setView }) => (
    <View style={{flexDirection:'row',alignItems:'center',gap:8,marginBottom:10,flexWrap:'wrap'}}>
      <View>
        <TouchableOpacity ref={yearBtnRef} style={rpt.yearBtn} onPress={()=>setYearDropdown(true)} activeOpacity={0.80}>
        ...
        {yearDropdown&&(
          <View style={rpt.yearMenu}>
            <ScrollView style={{maxHeight:180}} showsVerticalScrollIndicator={false}>
              {years.map(y=>(
                <TouchableOpacity key={y} style={[rpt.yearOpt,y===year&&rpt.yearOptActive]} onPress={()=>{setYear(y);setYearDropdown(false);}}>
      ...
      {[['daily','Daily'],['monthly','Monthly'],['yearly','Yearly']].map(([k,l])=>(
        <TouchableOpacity key={k} style={[rpt.viewToggleBtn,view===k&&rpt.viewToggleBtnActive]} onPress={()=>setView(k)}>
          <Text style={[rpt.viewToggleTxt,view===k&&rpt.viewToggleTxtActive]}>{l}</Text>
        </TouchableOpacity>
      ))}
    </View>
  );
  ...
  const MonthTabs = () => (
    <View style={{flexDirection:'row',flexWrap:'wrap',gap:5,marginBottom:10}}>
      {MONTHS.map((m,i)=>(
        <TouchableOpacity key={m} style={[rpt.monthBtn,activeMonth===i&&rpt.monthBtnActive]} onPress={()=>setActiveMonth(i)}>
  ...
  const TableHeader = ({ cols }) => (
    <View style={rpt.thead}>
      {cols.map((c,i)=><Text key={i} style={[rpt.th,c.style]}>{c.label}</Text>)}
    </View>
  );
  ...
  return (
    <View style={sub.root}>
      <View style={rpt.folderTabBar}>
        {FOLDERS.map((f,i)=>(
          <TouchableOpacity key={f.key} onPress={()=>setActiveFolder(f.key)}
            style={[rpt.folderTab,activeFolder===f.key&&rpt.folderTabActive,i===0&&{marginLeft:0}]} activeOpacity={0.80}>
            <Text style={[rpt.folderTabTxt,activeFolder===f.key&&rpt.folderTabTxtActive]}>{f.icon}  {f.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      <View style={rpt.folderCard}>
        {activeFolder==='sales'&&(
          <View style={{flex:1,minHeight:0}}>
            ...
            {salesView==='daily'&&(
              ...
              <View style={{backgroundColor:'rgba(26,58,107,0.10)',borderRadius:10,padding:10,marginBottom:10,flexDirection:'row',gap:8}}>
                <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:16,color:'#1a3a6b'}}>₱{monthTotal.toLocaleString('en-PH',{minimumFractionDigits:2})}</Text>
                <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:'rgba(1,31,75,0.50)',marginTop:2}}>{MONTHS_FULL[activeMonth]} {year} Total  ·  {monthOrders.length} orders</Text>
              </View>
            )}
            <TableHeader cols={[
              {label:'DATE/PERIOD',style:{flex:1.2,textAlign:'left',paddingLeft:10}},
              {label:'ORDERS',style:{flex:0.7}},
              {label:'TOTAL',style:{flex:1.1}},
            ]}/>
            <WebScrollView style={{flex:1}} contentContainerStyle={{paddingBottom:16}}>
              {(salesView==='daily'?dailyData:salesView==='monthly'?monthlyData:yearlyData).map((g,idx)=>(
                <View key={idx} style={[rpt.trow,idx%2===0&&rpt.trowAlt]}>
                  <Text style={[rpt.td,{flex:1.2,fontFamily:'GoogleSans_700Bold',color:'#1a3a6b',textAlign:'left',paddingLeft:10}]}>
                    {salesView==='daily'?g.key:salesView==='monthly'?g.monthFull:g.year}
                  </Text>
                  <Text style={[rpt.td,{flex:0.7}]}>{g.count}</Text>
                  <Text style={[rpt.td,{flex:1.1,fontFamily:'GoogleSans_700Bold',color:'#c9a84c'}]}>₱{g.total.toLocaleString('en-PH',{minimumFractionDigits:2})}</Text>
                </View>
              ))}
              {(salesView==='daily'?dailyData:salesView==='monthly'?monthlyData:yearlyData).length===0&&(
                <View style={rpt.emptyRow}><Text style={rpt.emptyTxt}>No sales data for this period.</Text></View>
              )}
            </WebScrollView>
          </View>
        )}
        ...
        {activeFolder==='tx'&&(
          <View style={{flex:1,minHeight:0}}>
            ...
            <TableHeader cols={[
              {label:'DATE/PERIOD',style:{flex:1.0,textAlign:'left',paddingLeft:10}},
              {label:'TOTAL ORDERS',style:{flex:1.0}},
              {label:'TOTAL EARNINGS',style:{flex:1.2}},
            ]}/>
            <WebScrollView style={{flex:1}} contentContainerStyle={{paddingBottom:16}}>
              {(txView==='daily'?dailyData:txView==='monthly'?monthlyData:yearlyData).map((g,idx)=>(
                <View key={idx} style={[rpt.trow,idx%2===0&&rpt.trowAlt]}>
                  <Text style={[rpt.td,{flex:1.0,fontFamily:'GoogleSans_700Bold',color:'#1a3a6b',textAlign:'left',paddingLeft:10}]}>
                    {txView==='daily'?g.key:txView==='monthly'?g.monthFull+' '+year:g.year}
                  </Text>
                  <Text style={[rpt.td,{flex:1.0}]}>{g.count}</Text>
                  <Text style={[rpt.td,{flex:1.2,fontFamily:'GoogleSans_700Bold',color:'#1a7a45'}]}>₱{g.total.toFixed(2)}</Text>
                </View>
              ))}
              {(txView==='daily'?dailyData:txView==='monthly'?monthlyData:yearlyData).length===0&&(
                <View style={rpt.emptyRow}><Text style={rpt.emptyTxt}>No transaction data for this period.</Text></View>
              )}
            </WebScrollView>
          </View>
        )}
        ...
        {activeFolder==='inv'&&(
          <View style={{flex:1,minHeight:0}}>
            ...
            <View style={{backgroundColor:'rgba(26,58,107,0.10)',borderRadius:10,padding:12,marginBottom:10,flexDirection:'row',gap:16,flexWrap:'wrap'}}>
              <View style={{gap:2}}>
                <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:9,color:'rgba(1,31,75,0.45)',letterSpacing:1,textTransform:'uppercase'}}>Total Items</Text>
                <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:18,color:'#1a3a6b'}}>{items.length}</Text>
              </View>
              <View style={{gap:2}}>
                <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:9,color:'rgba(1,31,75,0.45)',letterSpacing:1,textTransform:'uppercase'}}>Total Stock</Text>
                <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:18,color:'#1a3a6b'}}>{items.reduce((s,i)=>s+(i.stock||0),0)}</Text>
              </View>
              <View style={{gap:2}}>
                <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:9,color:'rgba(1,31,75,0.45)',letterSpacing:1,textTransform:'uppercase'}}>Total Value</Text>
                <Text style={{fontFamily:'NotoSerif_700Bold',fontSize:18,color:'#c9a84c'}}>₱{items.reduce((s,i)=>s+(i.price||0)*(i.stock||0),0).toLocaleString()}</Text>
              </View>
            </View>
            <TableHeader cols={[
              {label:'ITEM',style:{flex:2,textAlign:'left',paddingLeft:10}},
            ...
            <WebScrollView style={{flex:1}} contentContainerStyle={{paddingBottom:16}}>
              {items.map((item,idx)=>(
                <View key={item.id} style={[rpt.trow,idx%2===0&&rpt.trowAlt]}>
                  <Text style={[rpt.td,{flex:2,textAlign:'left',paddingLeft:10,fontFamily:'GoogleSans_700Bold',color:'#1a2d4e'}]} numberOfLines={1}>{item.emoji} {item.name}</Text>
                  <Text style={[rpt.td,{flex:1}]}>{item.cat}</Text>
                  <Text style={[rpt.td,{flex:0.7,color:item.stock===0?'#e74c3c':item.stock<=5?'#b85c00':'#1a2d4e',fontFamily:'GoogleSans_700Bold'}]}>{item.stock}</Text>
                  <Text style={[rpt.td,{flex:0.9}]}>₱{item.price}</Text>
                  <Text style={[rpt.td,{flex:1,fontFamily:'GoogleSans_700Bold',color:'#1a3a6b'}]}>₱{(item.price*item.stock).toLocaleString()}</Text>
                </View>
              ))}
              {items.length===0&&<View style={rpt.emptyRow}><Text style={rpt.emptyTxt}>No items in inventory.</Text></View>}
            </WebScrollView>
          </View>
        )}
      </View>
...
const rpt = StyleSheet.create({
  folderTabBar: { flexDirection:'row',flexShrink:0,marginTop:0,paddingHorizontal:0,alignItems:'flex-end',gap:2 },
  folderTab: { paddingVertical:8,paddingHorizontal:14,borderTopLeftRadius:10,borderTopRightRadius:10,backgroundColor:'rgba(26,58,107,0.45)',borderWidth:1,borderBottomWidth:0,borderColor:'rgba(255,255,255,0.20)',marginRight:2 },
  folderTabActive: { backgroundColor:'rgba(240,245,249,0.97)',borderColor:'rgba(255,255,255,0.55)',paddingBottom:10 },
  folderTabTxt: { fontFamily:'GoogleSans_700Bold',fontSize:11,color:'rgba(255,255,255,0.80)' },
  folderTabTxtActive: { color:'#1a3a6b' },
  folderCard: { flex:1,minHeight:0,backgroundColor:'rgba(240,245,249,0.97)',borderRadius:14,borderTopLeftRadius:0,borderWidth:1,borderColor:'rgba(255,255,255,0.55)',padding:12 },
  yearBtn: { flexDirection:'row',alignItems:'center',gap:8,paddingVertical:6,paddingHorizontal:14,backgroundColor:'rgba(26,58,107,0.12)',borderRadius:8,borderWidth:1.5,borderColor:'rgba(26,58,107,0.20)' },
  ...
  yearMenu: { position:'absolute',top:36,left:0,zIndex:9999,backgroundColor:'#ffffff',borderRadius:10,borderWidth:1,borderColor:'rgba(26,58,107,0.18)',shadowColor:'#000',shadowOpacity:0.18,shadowRadius:12,elevation:20,minWidth:120,maxHeight:180 },
...
const OrderingMonitoring = ({ orders, onUpdateStatus }) => {
  ...
  const doneToday      = orders.filter(o=>{
    ...
    try{
      const ts = o.createdAt?.toDate?.() || new Date(o.createdAt || o.time);
  ...
  const filteredOrders = orders.filter(o => {
    if(activeFilter==='done'){
      if(o.status!=='done')return false;
      try{const ts=o.createdAt?.toDate?.()||new Date(o.createdAt||o.time||0);return ts.toDateString()===new Date().toDateString();}catch{return false;}
    }
            ...
            return rows.map((row,rIdx)=>(
              <View key={rIdx} style={{flexDirection:'row',gap:6}}>
                {row.map(order=>{
                  ...
                  const ts=order.createdAt?.toDate?.()||new Date(order.createdAt||order.time||0);
                  const timeStr=isNaN(ts)?'':ts.toLocaleTimeString('en-PH',{hour:'2-digit',minute:'2-digit'});
                  return(
                    <View key={order.id||order.docId} style={[lp.card,{flex:1,borderTopColor:cfg.color}]}>
                      <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
                        <Text style={lp.cardId}>#{order.orderNo||order.id?.slice(-4)||'--'}</Text>
                        <Text style={lp.cardTime}>{timeStr}</Text>
                      </View>
                      {order.memberName&&<Text style={{fontFamily:'GoogleSans_700Bold',fontSize:8,color:'#1a3a6b'}}>👤 {order.memberName}</Text>}
                      <Text style={lp.cardItems} numberOfLines={3}>{itemsList}</Text>
                      <Text style={lp.cardTotal}>{'₱'}{Number(order.total).toFixed(0)}</Text>
                      <Text style={lp.cardPay}>{order.payment==='gcash'?'📱 GCash':order.payment==='credit'?'🪙 Credit':'💵 Cash'}</Text>
                      {cfg.btnLabel&&(
                        <TouchableOpacity style={[lp.actionBtn,{backgroundColor:cfg.btnColor}]}
                          onPress={()=>onUpdateStatus(order.id||order.docId,cfg.next)} activeOpacity={0.80}>
                ...
                {Array.from({length:3-row.length}).map((_,i)=>(<View key={`e-${i}`} style={{flex:1}}/>))}
              </View>
            ));
          })()
        }
...
export default function ManageGroceryScreen({ navigation }) {
  ...
  const saveItem = async (item) => {
    ...
    } catch (e) { Alert.alert('Error', 'Could not save item: ' + e.message); }
  };

  const deleteItem = async (id) => {
    try { await deleteDoc(doc(db, 'grocery_items', id)); }
  ...
  const addOrder = async (order) => {
    try {
      const ref = await addDoc(collection(db, 'grocery_orders'), {
        ...order, createdAt: serverTimestamp(), status: order.status || 'pending',
      });
      return ref.id;
    } catch (e) { console.warn('addOrder (grocery) error:', e.message); return null; }
  };
  ...
  const deductStock = async (orderItems) => {
    ...
    } catch (e) { console.warn('deductStock (grocery) error:', e.message); }
  };

  const updateOrderStatus = async (orderId, newStatus) => {
    try { await updateDoc(doc(db, 'grocery_orders', orderId), { status: newStatus }); }
    catch (e) { console.warn('updateOrderStatus (grocery) error:', e.message); }
  };
  ...
  const saveAd = async (ad) => {
    ...
    } catch (e) { Alert.alert('Error', 'Could not save ad: ' + e.message); }
  };

  const deleteAd = async (id) => {
    try { await deleteDoc(doc(db, 'grocery_ads', id)); }
    catch (e) { setAds(prev => prev.filter(a => a.id !== id)); }
  };
  ...
  useEffect(() => {
    ...
    const newOrders = orders.filter(
      o => !prevOrderIdsRef.current.has(o.id || o.docId) && (o.status === 'pending' || !o.status) && o.source !== 'cashier'
    );
    if (newOrders.length > 0) {
      const latest = newOrders[0];
      playOrderSound();
      setNotifBanner({ orderNo: latest.orderNo || latest.id, source: latest.memberName || latest.source || 'customer', total: latest.total || 0 });
      Animated.sequence([
  ...
  return (
    <View style={styles.root}>
      ...
      {notifBanner && (
        <Animated.View style={{ position:'absolute', top:Platform.OS==='web'?12:44, left:16, right:16, zIndex:999, transform:[{translateY:notifAnim}] }}>
          <LinearGradient colors={['#1a6b3a','#27ae60']} start={{x:0,y:0}} end={{x:1,y:0}}
            style={{ borderRadius:14, paddingHorizontal:16, paddingVertical:12, flexDirection:'row', alignItems:'center', gap:12, shadowColor:'#000', shadowOpacity:0.35, shadowRadius:12, shadowOffset:{width:0,height:4}, elevation:16, borderWidth:1, borderColor:'rgba(201,168,76,0.40)' }}>
            <Text style={{fontSize:24}}>🔔</Text>
            <View style={{flex:1}}>
              <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:13,color:'#fff',letterSpacing:0.3}}>New Grocery Order #{notifBanner.orderNo}</Text>
              <Text style={{fontFamily:'GoogleSans_400Regular',fontSize:11,color:'rgba(255,255,255,0.75)',marginTop:1}}>From: {notifBanner.source}  ·  ₱{Number(notifBanner.total).toFixed(2)}</Text>
            </View>
          </LinearGradient>
        </Animated.View>
      )}
      ...
      <Animated.View style={[styles.body,{opacity:bodyFade,flex:1,minHeight:0}]}>
        <View style={{ flex:1, flexDirection:isWide?'row':'column', minHeight:0, overflow:'hidden' }}>
          ...
          {isWide ? (
            ...
            <View style={[styles.leftPanelMobile, salesCollapsed&&{height:36}, isTablet&&!salesCollapsed&&{height:170}]}>
              <TouchableOpacity onPress={()=>setSalesCollapsed(v=>!v)}
                style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:12,paddingVertical:6,backgroundColor:'rgba(26,58,107,0.15)',borderBottomWidth:salesCollapsed?0:1,borderColor:'rgba(255,255,255,0.30)'}}
                activeOpacity={0.80}>
                <View style={{flexDirection:'row',alignItems:'center',gap:6}}>
                  <View style={{width:6,height:6,borderRadius:3,backgroundColor:'#e74c3c'}}/>
                  <Text style={{fontFamily:'GoogleSans_700Bold',fontSize:10,color:'#1a2d4e',letterSpacing:1.2}}>ORDER MONITORING</Text>
                </View>
                <MaterialIcons name={salesCollapsed?'expand-more':'expand-less'} size={18} color="rgba(26,58,107,0.60)"/>
              </TouchableOpacity>
              {!salesCollapsed&&<View style={{flex:1,minHeight:0,overflow:'hidden'}}><OrderingMonitoring orders={orders} onUpdateStatus={updateOrderStatus}/></View>}
            </View>
          )}
            ...
            <View style={styles.adWrapper}>
              <ScrollView ref={adScrollRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false}
                ...
                {ads.map(ad=>{
                  ...
                  return(
                    <LinearGradient key={ad.id} colors={ad.bg||['#1a6b3a','#27ae60']} start={{x:0,y:0}} end={{x:1,y:1}} style={[styles.adSlide,{width:bannerW}]}>
                      ...
                      <TouchableOpacity style={styles.adEditBtn} onPress={async()=>{
                        if(Platform.OS==='web'){
                          if(window.confirm('Delete this ad?')){ await deleteAd(ad.id); }
                        } else {
                          Alert.alert('Delete Ad','Remove this ad?',[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:()=>deleteAd(ad.id)}]);
                        }
                      }}>
                ...
                <TouchableOpacity style={[styles.adSlide,{width:bannerW,backgroundColor:'rgba(26,58,107,0.18)',justifyContent:'center',alignItems:'center',gap:8,borderWidth:2,borderColor:'rgba(255,255,255,0.40)',borderStyle:'dashed'}]}
                  onPress={async()=>{
                    const title=Platform.OS==='web'?window.prompt('Ad Title:','Today\'s Deals'):'New Ad';
                    if(!title)return;
                    const sub=Platform.OS==='web'?window.prompt('Ad Subtitle:','Fresh groceries available!'):'';
                    const newAd={id:Date.now().toString(),title:title||'New Ad',sub:sub||'',emoji:'🛒',bg:['#1a6b3a','#27ae60'],imageUrl:'',image:null,target:'both',url:''};
                    await saveAd(newAd);
                  }}>
