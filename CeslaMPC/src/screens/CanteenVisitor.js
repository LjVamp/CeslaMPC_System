...
const LocationPickerModal = ({ visible, onClose, onConfirm, deliveryType }) => {
  ...
  // CDO bounding box for filtering
  const CDO_BBOX = '124.55,8.37,124.78,8.58'; // minLng,minLat,maxLng,maxLat
  ...
  const handleSearchChange = (text) => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        // Fallback to Geoapify if Photon returns nothing
        if (results.length === 0) {
          results = await searchGeoapify(text);
        }
        setSuggestions(results);
      } catch (_) {
        // If Photon fails, try Geoapify
        try {
  ...
  // ── No place_id to resolve (both APIs return coords directly) ────────────
...
const CartPanel = ({ cart, onAdd, onRemove, onClear, onOrder, onPlaceOrder, isWide, hideTitle, lastOrder, onShowReceipt }) => {
  ...
  const DELIVERY_BASE = 15;   // ₱15 base fee
  const DELIVERY_PER_KM = 5;  // ₱5 per km
