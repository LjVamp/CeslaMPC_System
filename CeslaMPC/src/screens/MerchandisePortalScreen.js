// src/screens/MerchandisePortalScreen.js
// CESLA MPC — Merchandise Portal
// Member/Visitor chooser — mirrors CanteenScreen.js design exactly
// Navigate here when user taps "Merchandise" from HomeScreen
...
const ROLES = [
  {
    id: 'member',
    label: 'Member',
    description: 'Order merchandise with your\nCESLA member account',
    icon: '👤',
    badge: null,
    accent: '#6fa3f7',
    screen: 'MerchandiseMemberScreen',
  },
  {
    id: 'visitor',
    label: 'Visitor',
    description: 'Browse & order as a walk-in\nguest — no login needed',
    icon: '🚶',
    badge: 'Walk-in',
    badgeType: 'new',
    accent: '#b47aff',
...
// ─── MAIN SCREEN ──────────────────────────────────────────────────────────────
export default function MerchandisePortalScreen({ navigation }) {
    ...
    <View style={styles.root}>
      ...
      <Animated.View style={{
        ...
        <View style={[styles.header, {
          ...
          <View style={styles.headerCenter}>
            <Text
              style={[styles.headerH1, { fontSize: isWide ? 22 : isSmall ? 14 : 16 }]}
              numberOfLines={1}
      ...
      >
        <View style={[styles.main, { paddingHorizontal: PAD }]}>
          ...
          {/* Cards grid */}
          <View style={[styles.grid, { flexDirection: 'row', alignItems: 'stretch', gap: GAP }]}>
            {ROLES.map((role, i) => (
              <RoleCard
                key={role.id}
                role={role}
          ...
          <Animated.View style={[
            ...
            <Text style={[styles.taglineP, { fontSize: isWide ? 14 : 13 }]}>
              {'Welcome to '}
              <Text style={styles.taglineWhite}>CESLA Merchandise Store!</Text>
              {'  •  Quality products, fair prices.'}
            </Text>
            <Text style={[styles.taglineP, { fontSize: isWide ? 14 : 13 }]}>
              {'Members enjoy '}
              <Text style={styles.taglineHighlight}>exclusive benefits</Text>
              {' — '}
              <Text style={styles.taglineGoldLight}>order with your account.</Text>
            </Text>
          </Animated.View>
        </View>
        ...
        <Animated.View style={[styles.footer, { opacity: ftFade }]}>
          ...
          <Text style={[styles.footerP, { fontSize: isWide ? 11 : 11 }]}>
            CESLA Multi-Purpose Cooperative  •  Merchandise Ordering System  •  2025
          </Text>
        </Animated.View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  badgeAdmin: { backgroundColor: '#c0392b' },
  badgeNew:   { backgroundColor: '#27ae60' },
  badgeText: {
