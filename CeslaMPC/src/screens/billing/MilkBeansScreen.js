      ...
      <View style={{ flex:1, backgroundColor:PAGE_BG }}>
        ...
        <View style={[s.deptSection, { maxHeight: wh * 0.55 }]}>
          <View style={s.thead}>
            ...
            <Text style={[s.th, { flex:1.5 }]}>MILK</Text>
            <Text style={[s.th, { flex:1.5 }]}>BEANS</Text>
            ...
            {detailEntries.length === 0
              ...
              : detailEntries.map(item => (
                <View key={item.id} style={s.row}>
                  ...
                  <Text style={[s.td, { flex:1.5, textAlign:'center', fontSize:11 }]}>{item.milkType !== 'None' ? `${item.milkQty} × ${fmt(item.milkPrice)}` : '—'}</Text>
                  <Text style={[s.td, { flex:1.5, textAlign:'center', fontSize:11 }]}>{item.beansType !== 'None' ? `${item.beansQty} × ${fmt(item.beansPrice)}` : '—'}</Text>
