...
export default function MilkBeansScreen({ year, month }) {
  ...
  const dates = useMemo(
    () => getUniqueDates('milkbeans', year, month),
    [entries, year, month]
  );
  ...
  const buildHtml = async (row) => {
    ...
    DEPARTMENTS.forEach(function(dept) {
      getEntriesByDate('milkbeans', rowYear, rowMonth, row.date)
        .filter(function(e) { return e.dept === dept; })
        .forEach(function(e) {
          const a  = e.amount || 0;
          gt += a;
          const mQ = e.milkType  !== 'None' ? e.milkQty  : '\u2014';
          const mP = e.milkType  !== 'None' ? Number(e.milkPrice  || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 }) : '\u2014';
          const bQ = e.beansType !== 'None' ? e.beansQty : '\u2014';
          const bP = e.beansType !== 'None' ? Number(e.beansPrice || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 }) : '\u2014';
          tRows = tRows
            + '<tr>'
            + '<td style="' + cs + '">' + dateShort + '</td>'
            + '<td style="' + cs + ';font-weight:bold;text-align:center;">' + dept + '</td>'
            + '<td style="' + cs + ';text-align:center;">' + mQ + '</td>'
            + '<td style="' + cs + ';text-align:center;">' + mP + '</td>'
            + '<td style="' + cs + ';text-align:center;">' + bQ + '</td>'
            + '<td style="' + cs + ';text-align:center;">' + bP + '</td>'
            + '<td style="' + cs + ';text-align:right;">' + Number(a).toLocaleString('en-PH', { minimumFractionDigits: 2 }) + '</td>'
            + '</tr>';
        });
    });

    if (!tRows) tRows = '<tr><td colspan="7" style="text-align:center;color:#999;padding:10px;">No entries</td></tr>';
    ...
    const para = ''
      ...
      + 'This is to bill CLIMBS LIFE &amp; GENERAL INSURANCE COOPERATIVE for the Milk &amp; Beans Supply dated '
      + dateStr + paid
      + '</div>';
    ...
    const tbl = ''
      ...
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Milk QTY</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Milk Price</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Beans QTY</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Beans Price</th>'
      + '<th style="padding:8px 10px;text-align:right;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">AMOUNT</th>'
      + '</tr></thead>'
      + '<tbody>' + tRows + '</tbody>'
      + '<tfoot><tr>'
      + '<td colspan="6" style="border:1px solid #aab4c8;border-top:2px solid #1a2a4a;padding:7px 10px;text-align:right;font-weight:bold;color:#1a2a4a;background:#dce3f0;-webkit-print-color-adjust:exact;print-color-adjust:exact">GRAND TOTAL:</td>'
      ...
      <View style={{ flex: 1, backgroundColor: PAGE_BG }}>
        ...
        <View style={[s.deptSection, { maxHeight: winHeight * 0.55 }]}>
          <View style={s.detailThead}>
            ...
            <Text style={[s.th, { flex: 1.5, textAlign: 'center' }]}>MILK</Text>
            <Text style={[s.th, { flex: 1.5, textAlign: 'center' }]}>BEANS</Text>
            ...
            {detailEntries.length === 0
              ...
              : detailEntries.map(item => (
                <View key={item.id} style={s.detailRow}>
                  ...
                  <Text style={[s.td, { flex: 1.5, textAlign: 'center', fontSize: 11 }]}>
                    {item.milkType !== 'None' ? item.milkQty + ' x ' + fmt(item.milkPrice) : '\u2014'}
                  </Text>
                  <Text style={[s.td, { flex: 1.5, textAlign: 'center', fontSize: 11 }]}>
                    {item.beansType !== 'None' ? item.beansQty + ' x ' + fmt(item.beansPrice) : '\u2014'}
                  </Text>
