...
export default function TicketScreen({ year, month }) {
  ...
  const dates = useMemo(
    () => getUniqueDates('ticket', year, month),
    [entries, year, month]
  );
  ...
  const buildHtml = async (row) => {
    ...
    let tRows = '';
    let gt = 0;
    getEntriesByDate('ticket', rowYear, rowMonth, row.date).forEach(function(e) {
      const a = e.amount || 0;
      gt += a;
      tRows = tRows
        + '<tr>'
        ...
        + '<td style="' + cs + ';text-align:center;">' + (e.tickets || 0) + '</td>'
        + '<td style="' + cs + ';text-align:center;">' + Number(e.amtTicket || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 }) + '</td>'
    ...
    const para = ''
      ...
      + 'This is to bill CLIMBS LIFE &amp; GENERAL INSURANCE COOPERATIVE for the Ticket dated '
      + dateStr + paid
      + '</div>';
    ...
    const tbl = ''
      ...
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">No. of Ticket</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Price/Ticket</th>'
      ...
      <View style={{ flex: 1, backgroundColor: PAGE_BG }}>
        ...
        <View style={[s.deptSection, { maxHeight: winHeight * 0.55 }]}>
          <View style={s.detailThead}>
            ...
            <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>TICKETS</Text>
            <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>AMT/TKT</Text>
            ...
            {detailEntries.length === 0
              ...
              : detailEntries.map(item => (
                <View key={item.id} style={s.detailRow}>
                  ...
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{fmt(item.amtTicket)}</Text>
    ...
    <View style={{ flex: 1, backgroundColor: PAGE_BG }}>
      ...
      <View style={[s.deptSection, { maxHeight: winHeight * 0.55 }]}>
        <View style={s.thead}>
          ...
          <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>AMT/TICKET</Text>
            ...
            : filteredRows.map(d => (
              <View key={d.date} style={[s.row, searchQuery && s.rowHighlight]}>
                ...
                >
                  ...
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{fmt(d.amtTicket)}</Text>
