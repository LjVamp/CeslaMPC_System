...
export default function RiceAllowancesScreen({ year, month }) {
  ...
  const dates = useMemo(() => getUniqueDates('riceallowances', year, month), [entries, year, month]);
      ...
      <View style={{ flex:1, backgroundColor:PAGE_BG }}>
        ...
        <View style={[s.deptSection, { maxHeight: wh * 0.55 }]}>
          <View style={s.thead}>
            ...
            <Text style={[s.th, { flex:1 }]}>SACKS</Text>
            <Text style={[s.th, { flex:1.4 }]}>PRICE/SAC</Text>
            ...
            {detailEntries.length === 0
              ...
              : detailEntries.map(item => (
                <View key={item.id} style={s.row}>
                  ...
                  <Text style={[s.td, { flex:1.4, textAlign:'center' }]}>{fmt(item.priceSac)}</Text>
    ...
    <View style={{ flex:1, backgroundColor:PAGE_BG }}>
      ...
      <View style={[s.deptSection, { maxHeight: wh * 0.55 }]}>
        <View style={s.thead}>
          ...
          <Text style={[s.th, { flex:1.2, textAlign:'center' }]}>PRICE/SAC</Text>
          ...
          {rows.length === 0
            ...
            : rows.map(d => (
              <TouchableOpacity key={d.date} style={s.row} onPress={() => setDetailDate(d.date)} activeOpacity={0.75}>
                ...
                <Text style={[s.td, { flex:1.2, textAlign:'center' }]}>{fmt(d.priceSac)}</Text>
