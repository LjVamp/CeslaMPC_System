...
export default function RiceAllowancesScreen({ year, month }) {
  ...
  const dates = useMemo(
    () => getUniqueDates('riceallowances', year, month),
    [entries, year, month]
  );

  const rows = useMemo(() => dates.map((date, idx) => {
    const dayEntries = getEntriesByDate('riceallowances', year, month, date);
    const totalAmt   = dayEntries.reduce((s, e) => s + (e.amount || 0), 0);
    const totalSacks = dayEntries.reduce((s, e) => s + (parseFloat(e.sacks) || 0), 0);
    const priceSac   = dayEntries[0]?.priceSac || 0;
    const status     = dayEntries[0]?.status || 'pending';
    const mm         = String(month + 1).padStart(2, '0');
    const billingNo  = 'RA ' + year + '-' + mm + '-' + String(idx + 1).padStart(2, '0');
    return { date, totalAmt, totalSacks, priceSac, status, billingNo, id: dayEntries[0]?.id };
  }), [dates, entries]);

  const grandTotal    = rows.reduce((s, r) => s + r.totalAmt, 0);
  const detailEntries = useMemo(
    () => detailDate ? getEntriesByDate('riceallowances', year, month, detailDate) : [],
    [detailDate, entries]
  );
  ...
  const buildHtml = async (row) => {
    ...
    let tRows = '';
    let gt = 0;
    getEntriesByDate('riceallowances', rowYear, rowMonth, row.date).forEach(function(e) {
      const a = e.amount || 0;
      gt += a;
      tRows = tRows
        + '<tr>'
        + '<td style="' + cs + '">' + dateShort + '</td>'
        + '<td style="' + cs + '">' + (e.name || '') + '</td>'
        + '<td style="' + cs + ';font-weight:bold;text-align:center;">' + (e.dept || '') + '</td>'
        + '<td style="' + cs + ';text-align:center;">' + (e.sacks || 0) + '</td>'
        + '<td style="' + cs + ';text-align:center;">' + Number(e.priceSac || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 }) + '</td>'
        + '<td style="' + cs + ';text-align:right;">' + Number(a).toLocaleString('en-PH', { minimumFractionDigits: 2 }) + '</td>'
        + '</tr>';
    });

    if (!tRows) tRows = '<tr><td colspan="6" style="text-align:center;color:#999;padding:10px;">No entries</td></tr>';
    ...
    const para = ''
      ...
      + 'This is to bill CLIMBS LIFE &amp; GENERAL INSURANCE COOPERATIVE for the Rice Allowance dated '
      + dateStr + paid
      + '</div>';
    ...
    const tbl = ''
      ...
      + '<th style="padding:8px 10px;text-align:left;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Name</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Department</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">No. of Sac</th>'
      + '<th style="padding:8px 10px;text-align:center;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Price/Sac</th>'
      + '<th style="padding:8px 10px;text-align:right;border:1px solid #aab4c8;color:#1a2a4a;font-weight:bold;">Amount</th>'
      + '</tr></thead>'
      + '<tbody>' + tRows + '</tbody>'
      + '<tfoot><tr>'
      + '<td colspan="5" style="border:1px solid #aab4c8;border-top:2px solid #1a2a4a;padding:7px 10px;text-align:right;font-weight:bold;color:#1a2a4a;background:#dce3f0;-webkit-print-color-adjust:exact;print-color-adjust:exact">GRAND TOTAL:</td>'
      ...
      <View style={{ flex: 1, backgroundColor: PAGE_BG }}>
        ...
        <View style={[s.deptSection, { maxHeight: winHeight * 0.55 }]}>
          <View style={s.detailThead}>
            ...
            <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>PRICE/SAC</Text>
            ...
            {detailEntries.length === 0
              ...
              : detailEntries.map(item => (
                <View key={item.id} style={s.detailRow}>
                  ...
                  <Text style={[s.td, { flex: 1.3, textAlign: 'center' }]}>{item.name || ''}</Text>
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{item.sacks}</Text>
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{fmt(item.priceSac)}</Text>
    ...
    <View style={{ flex: 1, backgroundColor: PAGE_BG }}>
      ...
      <View style={[s.deptSection, { maxHeight: winHeight * 0.55 }]}>
        <View style={s.thead}>
          ...
          <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>TOTAL SACKS</Text>
          <Text style={[s.th, { flex: 1, textAlign: 'center' }]}>PRICE/SAC</Text>
            ...
            : filteredRows.map(d => (
              <View key={d.date} style={[s.row, searchQuery && s.rowHighlight]}>
                ...
                >
                  ...
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{d.totalSacks}</Text>
                  <Text style={[s.td, { flex: 1, textAlign: 'center' }]}>{fmt(d.priceSac)}</Text>
