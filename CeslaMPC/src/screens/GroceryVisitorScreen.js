  ...
  const handleSearchChange = (text) => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        try { const results = await searchGeoapify(text); setSuggestions(results); }
      ...
      if (Platform.OS === 'web') {
        ...
        await new Promise((res, rej) => {
          navigator.geolocation.getCurrentPosition(
            ...
            e => rej(e), { enableHighAccuracy: true, timeout: 12000 }
          );
        });
      } else {
            ...
            {Platform.OS !== 'web' && MapView ? (
              <>
                <MapView
                  ...
                  showsUserLocation showsMyLocationButton={false} showsCompass showsScale
                >
                ...
                {coords && <View style={lpStyles.mapHint} pointerEvents="none"><View style={lpStyles.mapHintBubble}><Text style={lpStyles.mapHintTxt}>Drag pin to adjust</Text></View></View>}
              </>
            ) : (
              Platform.OS === 'web' ? (
                <iframe key={mapKey} ref={iframeRef} title="Location Map" srcDoc={LEAFLET_HTML(coords?.lat, coords?.lng, isDeliver)} style={{ width:'100%', height:'100%', border:0, borderRadius:14 }} sandbox="allow-scripts allow-same-origin" />
              ) : (
...
// ─── CART PANEL ───────────────────────────────────────────────────────────────
const CartPanel = ({ cart, onAdd, onRemove, onClear, onOrder, onPlaceOrder, isWide, hideTitle, lastOrder, onShowReceipt, orderHistory = [], liveOrders = [] }) => {
  ...
  // Use liveOrders (real-time from Firestore) to get accurate status — avoids stale AsyncStorage status
  const getLiveStatus = (order) => {
    const live = liveOrders.find(o => o.id === order.docId || o.docId === order.docId);
    return live?.status || order.status || 'done';
  };
  const pendingCount = orderHistory.filter(o => { const s = getLiveStatus(o); return s === 'pending' || s === 'preparing' || s === 'ready'; }).length;

  // ── Store/Grocery coords (origin for delivery distance) ─────────────────────
  const STORE_LAT = 8.4748;
  const STORE_LNG = 124.6465;
  ...
  const deliveryFee = (deliveryType === 'deliver' && deliveryCoords)
    ? Math.round(DELIVERY_BASE + getDistanceKm(STORE_LAT, STORE_LNG, deliveryCoords.lat, deliveryCoords.lng) * DELIVERY_PER_KM)
    : 0;
  ...
  return (
    ...
    <View style={[styles.cartPanel, !isWide && styles.cartPanelMobile]}>
      ...
      {/* ── Tab row: Cart | Placed Orders — only shown on wide/desktop panel ── */}
      {!hideTitle && (
      ...
      {activePanelTab === 'cart' && (<>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 200 }}>
              {cartItems.map(({ item, qty }) => (
                <View key={item.id} style={styles.cartRow}>
                  <TouchableOpacity style={[styles.checkbox, checked[item.id] && styles.checkboxChecked]} onPress={() => toggleCheck(item.id)}>
        ...
        {deliveryType === 'deliver' && (
          <View style={styles.totalRow}>
            <Text style={[styles.totalLabel, { fontSize:12, color: deliveryCoords ? '#e74c3c' : 'rgba(1,31,75,0.45)' }]}>
              🛵 Delivery Fee {deliveryCoords ? `(${getDistanceKm(STORE_LAT, STORE_LNG, deliveryCoords.lat, deliveryCoords.lng).toFixed(1)} km)` : ''}:
            </Text>
        ...
        {/* ── Place Order button ── */}
        <TouchableOpacity style={[styles.placeOrderBtn, checkedItems.length === 0 && styles.placeOrderBtnDisabled]} onPress={handlePlaceOrder} activeOpacity={checkedItems.length === 0 ? 1 : 0.80}>
          <LinearGradient colors={checkedItems.length > 0 ? ['#27ae60','#2ecc71'] : ['#aaa','#bbb']} start={{x:0,y:0}} end={{x:1,y:0}} style={styles.placeOrderGrad}>
        ...
        <TouchableOpacity style={[styles.printBtn, !lastOrder && { opacity: 0.45 }]} onPress={lastOrder ? onShowReceipt : null} activeOpacity={0.80}>
            ...
            orderHistory.map((order, idx) => {
              ...
              return (
                <View key={order.docId || idx} style={{
                  ...
                  shadowOpacity: isActive ? 0.15 : 0, shadowRadius:6, elevation: isActive ? 3 : 0,
                }}>
                    ...
                    {items.map((it, j) => {
                      ...
                      return (
                        <Text key={j} style={{ fontFamily:'GoogleSans_400Regular', fontSize:11, color:'#0f1e35', marginBottom:2, lineHeight:16 }} numberOfLines={1}>
                          {item.emoji || '🛒'} {item.name} x{qty} — ₱{(item.price * qty).toFixed(2)}
                        </Text>
                      );
                    })}
                  </View>
...
const CartBottomSheet = ({ cart, onAdd, onRemove, onClear, onOrder, onClose, onPlaceOrder, lastOrder, onShowReceipt, orderHistory = [], liveOrders = [] }) => {
        ...
        {activeSheetTab === 'cart' ? (
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow:1 }}>
            <CartPanel
              ...
              orderHistory={orderHistory} liveOrders={liveOrders}
            />
          </ScrollView>
        ) : (
              ...
              orderHistory.map((order, idx) => {
                ...
                return (
                  <View key={order.docId || idx} style={{
                    ...
                    borderColor: isActive ? stCfg.border : 'rgba(255,255,255,0.70)',
                    shadowColor: isActive ? stCfg.color : 'transparent',
                    shadowOpacity: isActive ? 0.15 : 0, shadowRadius: 8, elevation: isActive ? 3 : 0,
                  }}>
                    ...
                    <View style={{ flexDirection:'row', alignItems:'center', marginBottom:8, gap:6 }}>
                      ...
                      {order.deliveryType === 'deliver' && (
                        <View style={{ paddingHorizontal:8, paddingVertical:3, borderRadius:12, backgroundColor:'rgba(231,76,60,0.12)', borderWidth:1, borderColor:'rgba(231,76,60,0.35)' }}>
                          <Text style={{ fontFamily:'GoogleSans_700Bold', fontSize:9, color:'#c0392b', letterSpacing:0.8 }}>🛵 Delivery</Text>
                        </View>
                      )}
                    </View>
...
const AdBanner = ({ isWide, adAnim, ads, navigation }) => {
  ...
  // ── Filter: show only ads targeted to 'visitor' or 'both' ──────────────────
  const filteredAds = ads && ads.length > 0
    ? ads.filter(ad => !ad.target || ad.target === 'both' || ad.target === 'visitor')
    : [];
      ...
      <Animated.View style={{ flex:1, flexDirection:'row', alignItems:'stretch', opacity: bodyFade, minHeight:0, overflow: Platform.OS==='web' ? 'hidden' : 'visible' }}>
        ...
        {isWide && (
          <CartPanel
            ...
            orderHistory={orderHistory} liveOrders={orders}
          />
        )}
            ...
            <View style={{ flexDirection:'row', alignItems:'center', gap:6, flexWrap:'wrap' }}>
              ...
              <View style={{ position:'relative', zIndex:9999 }}>
                <TouchableOpacity style={histStyles.sortBtn} onPress={() => { setSortDropOpen(v=>!v); setYearDropOpen(false); }} activeOpacity={0.80}>
                  ...
                  <Text style={{ fontSize:10, color:'rgba(1,31,75,0.55)' }}>{sortDropOpen ? '▲' : '▼'}</Text>
                </TouchableOpacity>
                {sortDropOpen && (
        ...
        <View style={[histStyles.panel, { zIndex:100, flex:2 }]}>
          ...
          <View style={{ position:'relative', zIndex:9999, marginBottom:10, alignSelf:'flex-start' }}>
            <TouchableOpacity style={histStyles.yearBtn} onPress={() => { setYearDropOpen(v=>!v); setSortDropOpen(false); }} activeOpacity={0.80}>
              ...
              <Text style={{ fontSize:13, color:'#0f1e35' }}>{yearDropOpen ? '▲' : '▼'}</Text>
            </TouchableOpacity>
            {yearDropOpen && (
